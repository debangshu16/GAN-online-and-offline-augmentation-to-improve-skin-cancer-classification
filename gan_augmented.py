...
for disease in labels:
    generated_image_count[disease]=0

#neglected_x =[]
#neglected_y =[]
for batch_number in range(0,n):
    ...
    #batch_x=[]
    prob_vec = np.random.uniform(0,1,batch_size)
    for i in range(x_batch.shape[0]):
        if prob_vec[i] <= keep_prob:
            pass
            #batch_x.append(x_batch[i])
        else:
            y_vec = batch_y[i]
            y = index_to_labels[np.argmax(y_vec)]
            generator = generator_model[y]
            noise = np.random.normal(0,1,(1,200))
            generated_img = generator.predict(noise)
            generated_img = 0.5* generated_img + 0.5
            #neglected_x.append(x_batch[i].tolist())
            #neglected_y.append(y_vec.tolist())
            x_batch[i] = generated_img
            train_x[batch_index+i] = generated_img
            generated_image_count[y]+=1
            #print (generated_img.shape)
            #imsave('training_generated_images/{}_{}.png'.format(y,generated_image_count[y]),generated_img[0])

    #print x_batch.shape,batch_y.shape
    #s=s+x_batch.shape[0]
    #full_model.train_on_batch(x_batch,batch_y)

batch_index= n*batch_size
x_batch=train_x[n*batch_size:]
batch_y = train_y[n*batch_size:]
prob_vec = np.random.uniform(0,1,x_batch.shape[0])
for i in range(x_batch.shape[0]):
    if prob_vec[i] <=keep_prob:
        pass
    else:
...
#full_model.train_on_batch(x_batch,y_batch)

file_path="GAN_weights_2.h5"
...
try:
    ...
    #print s
with open('gan_history_2.pickle','wb') as f:
...
def plot_model_history(model_history):
    ...
    plt.show()
    fig.savefig('GAN_Training_curve_2.png')
...
def plot_confusion_matrix(cm, classes,
    ...
    plt.savefig("GAN_Confusion_Matrix_2.png")
